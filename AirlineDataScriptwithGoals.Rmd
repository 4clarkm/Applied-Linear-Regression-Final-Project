...
modelt <- glm(On_Time ~ Airline + Dep_Hour + Dist_Dep_Arr, data = msn, family = binomial(link = "logit"))
summary(modelt)

vif(modelt)

Anova(modelt, type = "III")

rocmodelt <- roc(msn$On_Time ~ fitted(modelt))
plot(rocmodelt)
auc(rocmodelt)
```

```{r}
#Goal 2: Which airport of the 3 (MSN, MKE, ORD) has the lowest rate of cancelled flights?
#----------------------------------------------
...
modelc <- glm(Cancelled ~ Dep_Airport + Airline + Month + Dist_Dep_Arr, data = df_airports, family = binomial)
summary(modelc)

vif(modelc)

Anova(modelc, type = "III")

rocmodelc <- roc(df_airports$Cancelled ~ fitted(modelc))
plot(rocmodelc)
auc(rocmodelc)
```
